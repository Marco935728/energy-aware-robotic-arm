...
Where $d_t$ is distance to target, $P_t = \sum_i |\tau_i \cdot 
\dot{\theta}_i|$ is instantaneous mechanical power ( energy integral W = ∫τθ̇dt) and $R_{success}$ is an 
awarded obtained  when the target is reached within 0.05m.
The  weights $w_1$ and $w_2$ control the balance between accuracy 
and efficiency.
